...
class Cell {
    ...
    updateVelocity() {
        var y = (velocity[1][this.num]+velocity[1][this.num + width + 1])/2;
        var x = (velocity[0][this.num + 1]+velocity[0][this.num])/2;

        var newX = Math.round((this.location[0]*size + x*(1/timestep))/size);
        var newY = Math.round((this.location[1]*size + y*(1/timestep))/size);

        console.log(x,y,newX,newY);

        Cells[newX + newY * width].setVelocity = [x,0,false];
        Cells[newX + newY * width].setVelocity = [y,1,false];
        Cells[newX + newY * width].setVelocity = [-1*x,2,false];
        Cells[newX + newY * width].setVelocity = [-1*y,3,false];

        /*
        this.setVelocity = [-1*x,0,false];
        this.setVelocity = [-1*y,1,false];
        this.setVelocity = [x,3,false];
        this.setVelocity = [y,4,false];
        */
    }
}
